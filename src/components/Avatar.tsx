...
interface AvatarProps {
    ...
    className?: string;
    isAdmin?: boolean;
    /** 头像尺寸（px），设置后会覆盖 className 中的宽高 */
    size?: number;
}

export default function Avatar({ username, className = '', isAdmin = false, size }: AvatarProps) {
    ...
    return (
        <div style={sizeStyle} className={`relative flex items-center justify-center bg-slate-100 dark:bg-slate-800 transition-all ${className} ${isAdmin ? 'ring-[3px] ring-amber-400 ring-offset-2 ring-offset-background-light dark:ring-offset-background-dark shadow-[0_0_20px_rgba(251,191,36,0.6)] z-10' : 'overflow-hidden rounded-full'}`}>
